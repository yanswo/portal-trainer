...
export default function ExamPreviewForm({ exam }: ExamPreviewFormProps) {
  ...
  async function handleSubmit() {
    ...
    // Dummy validation (randomly pass or fail)
    const score = Math.floor(Math.random() * 41) + 60; // Random score between 60-100
    const passed = score >= exam.passingScore;

    setResult({ passed, score: score.toFixed(1) });
    setSubmitting(false);
  }
      ...
      <Card>
        ...
        <CardContent>
          <p style={{ color: "var(--color-text-muted)", marginBottom: "1.5rem" }}>
            {exam.description || "Complete esta avaliação para obter seu certificado."}
          </p>
